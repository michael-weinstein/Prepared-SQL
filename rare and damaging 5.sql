...
-- -------------------------------------------------------
-- This version was modified to use MD5 hashes to join the tables, avoiding collisions in the rare case where two rows share a locus.
...
create table {SUBJECT}_rare_damage as select * from {SUBJECT}_rare_damage_temp left join {SUBJECT}VCF on varMD5 = varMD5VCF;  #creates a new table by combining the data from the temporary table generated at the beginning of this script with the locus-matched data from the VCF table of read data
...
ALTER TABLE {SUBJECT}_rare_damage ADD INDEX (varMD5), ADD INDEX (uniLoc), ADD INDEX (geneList);
